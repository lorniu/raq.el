;;; raq.el --- Adapter of the request clients -*- lexical-binding: t -*-
...
(defvar raq-debug nil)

(defvar raq-multipart-boundary "raq-boundary-=-=+O0o0O69Oo")

(defun raq-log (&rest _args)
  "Log something ARGS.")
             ...
             (let* ((tag (eieio-object-class client))
                    (failfn (lambda (status)
                              ...
                              (if (and (string-match-p "Operation timeout" (format "%s" status)) (cl-plusp retry))
                                  (progn (raq-log tag (format "request timeout, retrying (remains %d times)..." retry))
                                         (apply #'raq client url `(:retry ,(1- retry) ,@args)))
                                ;; failed finally
                                (raq-log tag (format "Request FAIL: (%s) %s" url status))
                                  ...
                                  (unless raq-stream-abort-flag
                                    (condition-case err
                                      ...
                                      (error
                                       ...
                                       (raq-log tag (format "Error in filter: (%s) %s" url err))
                                       (funcall failfn err)))))))
                    (donefn (lambda (raw)
                              (raq-log tag (format "✓ %s" url))
...
SYNC and RETRY and more."
  ...
  (let* ((inhibit-message t)
    ...
    ;; log
    (raq-log (eieio-object-class client)
      (format "> %s\n> %s" client url)
...
(cl-defmethod raq (&rest args)
  "Send a request with `raq-default-client'. See the generic method for ARGS and more."
